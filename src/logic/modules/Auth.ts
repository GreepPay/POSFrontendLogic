...
export default class Auth extends Common {
  ...
  public GetDefaultBusiness = () => {
    ...
    if (currentBusinessId) {
      const business = this.AuthUser?.businesses?.find(
        (business) => parseInt(business.id) == parseInt(currentBusinessId)
      );
  ...
  public SignIn = (formIsValid: boolean) => {
    if (formIsValid && this.SignInForm) {
      return $api.auth
        ...
        .catch((error: CombinedError) => {
          Logic.Common.showError(error, "Oops!", "error-alert");
          if (error.message?.toLowerCase().includes("not match our record")) {
            Logic.Auth.SignOut();
          }
          throw new Error(error.message);
        });
